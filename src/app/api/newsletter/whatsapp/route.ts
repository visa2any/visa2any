...
export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const { name, phone, countries, terms } = body
    ...
    return NextResponse.json({,      success: true,      message: 'Cadastro realizado com sucesso! Você receberá uma mensagem de confirmação em breve.',      subscriber: {,        id: subscriber.id,        name: subscriber.name,        phone: subscriber.phone,        countries: subscriber.countries
      }
    })

  } catch (error) {,    console.error('[WHATSAPP NEWSLETTER] Erro:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
_Para cancelar, responda SAIR_`
  ...
  const whatsappResponse = await fetch('http://localhost:3000/api/whatsapp/send', {,    method: 'POST',    headers: { 'Content-Type': 'application/json' },    body: JSON.stringify({,      to: phone,      message: message,      type: 'newsletter_welcome'
    })
  }),
  if (!whatsappResponse.ok) {,    throw new Error('Falha ao enviar mensagem de boas-vindas')
  },
  return true
}

// GET - Listar assinantes (admin only),export async function GET() {,  try {,    const subscribers = await prisma.whatsAppSubscriber.findMany({,      where: { isActive: true },      orderBy: { createdAt: 'desc' },      take: 100
    }),
