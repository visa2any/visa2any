...
interface PaymentRequest {,  clientId: string,  country: string,  consulate: string,  availableDates: string[]
...
// Multiplicadores por plano,const PLAN_MULTIPLIERS = {,  'BASIC': 1.0,  'PREMIUM': 2.2,  'VIP': 3.0
}
...
export async function POST(request: NextRequest) {,  try {,    const data: PaymentRequest = await request.json()
    ...
    // 2. Calcular custos detalhados,    const multiplier = PLAN_MULTIPLIERS[data.plan],    const baseFee = Math.round(countryFees.serviceFee * multiplier),    
...
export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const paymentId = searchParams.get('paymentId'),    const clientId = searchParams.get('clientId'),
    if (paymentId) {
