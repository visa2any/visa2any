...
interface WebhookEvent {,  id: string,  event: string,  data: any,  timestamp: string,  signature?: string
},
...
// GET - Listar webhooks configurados,export async function GET(request: NextRequest) {,  try {,    const url = new URL(request.url),    const affiliateId = url.searchParams.get('affiliateId'),
...
// POST - Configurar novo webhook,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const { affiliateId, url: webhookUrl, events = [], secret } = body,
    if (!affiliateId || !webhookUrl || !Array.isArray(events)) {,      return NextResponse.json({,        error: 'affiliateId, url e events são obrigatórios'
      }, { status: 400 })
    }
  ...
  } catch (error) {,    console.error('Erro ao configurar webhook:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  }
}

// PUT - Atualizar webhook,export async function PUT(request: NextRequest) {,  try {,    const body = await request.json(),    const { affiliateId, webhookId, ...updateData } = body,
    if (!affiliateId || !webhookId) {,      return NextResponse.json({,        error: 'affiliateId e webhookId são obrigatórios'
      }, { status: 400 })
    },
    const endpoints = webhookEndpoints.get(affiliateId) || []
    const index = endpoints.findIndex(e => e.id === webhookId),
    if (index === -1) {,      return NextResponse.json({,        error: 'Webhook não encontrado'
      }, { status: 404 })
    }

    // Atualizar endpoint,    endpoints[index] = { ...endpoints[index], ...updateData },    webhookEndpoints.set(affiliateId, endpoints),
    return NextResponse.json({,      data: {
        ...endpoints[index]
        secret: undefined
      }
  ...
  } catch (error) {,    console.error('Erro ao atualizar webhook:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  }
}

// DELETE - Remover webhook,export async function DELETE(request: NextRequest) {,  try {,    const url = new URL(request.url),    const affiliateId = url.searchParams.get('affiliateId'),    const webhookId = url.searchParams.get('webhookId'),
    if (!affiliateId || !webhookId) {,      return NextResponse.json({,        error: 'affiliateId e webhookId são obrigatórios'
      }, { status: 400 })
    },
    const endpoints = webhookEndpoints.get(affiliateId) || []
    const filteredEndpoints = endpoints.filter(e => e.id !== webhookId),
    if (filteredEndpoints.length === endpoints.length) {,      return NextResponse.json({,        error: 'Webhook não encontrado'
      }, { status: 404 })
    },
    webhookEndpoints.set(affiliateId, filteredEndpoints),
    return NextResponse.json({,      data: { message: 'Webhook removido com sucesso' }
    })

  } catch (error) {,    console.error('Erro ao remover webhook:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  }
}
...
// Função para testar webhook,async function testWebhook(endpoint: WebhookEndpoint): Promise<{ success: boolean; message: string }> {,  try {,    const testEvent: WebhookEvent = {,      id: 'test_event',      event: 'webhook.test',      data: {,        message: 'Este é um evento de teste para verificar se seu webhook está funcionando corretamente.',        timestamp: new Date().toISOString()
    ...
    const payload = JSON.stringify(testEvent),    const signature = generateSignature(payload, endpoint.secret),
  ...
  } catch (error) {,    return { ,      success: false,      message: `Erro ao testar webhook: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
    }
  }
}
...
// Função para enviar evento para webhooks,async function sendWebhookEvent(,  affiliateId: string, ,  eventType: string, ,  eventData: any
    ...
    // Enviar para todos os endpoints relevantes,    const promises = relevantEndpoints.map(async (endpoint) => {,      try {,        const payload = JSON.stringify(webhookEvent),        const signature = generateSignature(payload, endpoint.secret),
        ...
        if (!response.ok) {,          console.error(`Erro ao enviar webhook para ${endpoint.url}: ${response.status}`)
        }

      } catch (error) {,        console.error(`Erro ao enviar webhook para ${endpoint.url}:`, error)
      }
    }),
    await Promise.allSettled(promises)

  } catch (error) {,    console.error('Erro ao enviar webhooks:', error)
  }
}
...
async function sendPaymentWebhook(affiliateId: string, paymentData: any) {,  await sendWebhookEvent(affiliateId, 'payment.completed', {,    paymentId: paymentData.id,    affiliateId,    amount: paymentData.amount,    method: paymentData.method,    transactionId: paymentData.transactionId,    processedAt: paymentData.processedAt,    timestamp: new Date().toISOString()
  })
},
async function sendTierPromotionWebhook(affiliateId: string, tierData: any) {,  await sendWebhookEvent(affiliateId, 'tier.promoted', {,    affiliateId,    oldTier: tierData.oldTier,    newTier: tierData.newTier,    newCommissionRate: tierData.newCommissionRate,    reason: tierData.reason,    timestamp: new Date().toISOString()
  })
},
async function sendClickWebhook(affiliateId: string, clickData: any) {,  await sendWebhookEvent(affiliateId, 'click.registered', {,    clickId: clickData.id,    affiliateId,    referralCode: clickData.referralCode,    url: clickData.url,    country: clickData.country,    device: clickData.device,    source: clickData.source,    campaign: clickData.campaign,    timestamp: new Date().toISOString()
  })
}