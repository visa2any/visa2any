...
async function getLocationFromIP(ip: string) {
  try {
    ...
    return {
      ...
      city: data.city || 'Unknown'}} catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL(targetUrl, request.url))} finally {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calcular comissão
    const commissionRate = affiliate.commissionRate || 0.10
