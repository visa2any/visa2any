...
// GET - Listar afiliados (Admin),export async function GET(request: NextRequest) {,  try {,    const url = new URL(request.url),    const page = parseInt(url.searchParams.get('page') || '1'),    const limit = parseInt(url.searchParams.get('limit') || '10'),    const status = url.searchParams.get('status'),    const tier = url.searchParams.get('tier'),    const search = url.searchParams.get('search'),
...
// POST - Criar novo afiliado,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const {,      name,      email,      phone,      company,      website,      bio,      socialMedia = {},      commissionRate = 0.10
    } = body
    ...
    // Gerar código de referência único,    let referralCode = generateReferralCode(name),    let attempts = 0,    
...
// PUT - Atualizar afiliado (Admin),export async function PUT(request: NextRequest) {,  try {,    const body = await request.json(),    const { id, ...updateData } = body,
