...
const defaultMaterials = [
  // Banners,  {,    type: 'BANNER',    title: 'Banner Principal - Visa2Any',    description: 'Banner principal com logo e call-to-action',    category: 'banners',    tags: ['principal', 'logo', 'cta']
    ...
    `,    imageUrl: '/images/banners/banner-principal.jpg',    previewUrl: '/materials/preview/banner-principal',    isActive: true
  },  {
    type: 'BANNER',    title: 'Banner Visto EUA',    description: 'Banner específico para promoção de visto americano',    category: 'banners',    tags: ['eua', 'visto', 'america']
    content: `
...
🇵🇹 Portugal
🇦🇺 Austrália
🇩🇪 Alemanha,E muito mais!

📞 Link na bio para consultoria gratuita
🎁 Use o código: {{REFERRAL_CODE}}

#morarfora #intercambio #imigração #visa2any #vivernoexterior #visto #consultoria #sonhoamericano`,    imageUrl: '/images/posts/instagram-consultoria.jpg',    isActive: true
  },  {
    type: 'SOCIAL_POST',    title: 'Post Facebook - Depoimento',    description: 'Post para Facebook com foco em depoimentos',    category: 'facebook',    tags: ['facebook', 'depoimento', 'sucesso']
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="padding: 30px 0;">
      ...
      <div style="text-align: center; padding: 20px;">
        <a href="{{AFFILIATE_LINK}}" style="background: #3B82F6; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: bold;">,          Agendar Consultoria Gratuita
        </a>
      </div>
      
      <p style="background: #F3F4F6; padding: 15px; border-radius: 8px; text-align: center;">
        <strong>Código especial: {{REFERRAL_CODE}}</strong><br>,        Use este código e ganhe condições exclusivas!
      </p>
    </div>
...
Consultoria gratuita: {{AFFILIATE_LINK}},Código especial: {{REFERRAL_CODE}}

© Visa2Any - Todos os direitos reservados
    `,    downloadUrl: '/downloads/guia-visto-eua.pdf',    imageUrl: '/images/guides/ebook-visto-eua.jpg',    isActive: true
  }
]
...
const affiliateId = url.searchParams.get('affiliateId')
  ...
  } catch (error) {,    console.error('Erro ao buscar materiais:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
...
const {,      type,      title,      description,      content,      category,      tags = []
  ...
  } catch (error) {,    console.error('Erro ao criar material:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  } finally {,    await prisma.$disconnect()
  }
}
...
const { id, ...updateData } = body,
    ...
    const material = await prisma.affiliateMaterial.update({,      where: { id },      data: {
  ...
  } catch (error) {,    console.error('Erro ao atualizar material:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  } finally {,    await prisma.$disconnect()
  }
}