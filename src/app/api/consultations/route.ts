...
// GET /api/consultations - Listar consultorias,export async function GET(request: NextRequest) {,  try {
    // Verificar autenticação,    const user = await verifyAuth(request),    if (!user) {,      return createAuthError('Acesso não autorizado')
    },    const { searchParams } = new URL(request.url),    const page = parseInt(searchParams.get('page') || '1'),    const limit = parseInt(searchParams.get('limit') || '10'),    const status = searchParams.get('status'),    const type = searchParams.get('type'),    const clientId = searchParams.get('clientId'),
...
// POST /api/consultations - Criar nova consultoria,export async function POST(request: NextRequest) {,  try {
    // Verificar autenticação,    const user = await verifyAuth(request),    if (!user) {,      return createAuthError('Acesso não autorizado')
