...
const skip = (page - 1) * limit
    ...
    const [consultations, total] = await Promise.all([,      prisma.consultation.findMany({,        where,        skip,        take: limit,        orderBy: { createdAt: 'desc' },        include: {,          client: {,            select: { ,              id: true, ,              name: true, ,              email: true, ,              phone: true,              targetCountry: true,              visaType: true,              status: true
            }
    ...
    return NextResponse.json({,      data: {,        consultations,        pagination: {,          page,          limit,          total,          totalPages,          hasMore: page < totalPages
        }
      }
    })

  } catch (error) {,    console.error('Erro ao buscar consultorias:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (client.status === 'LEAD' || client.status === 'QUALIFIED') {,      await prisma.client.update({,        where: { id: validatedData.clientId },        data: { status: 'CONSULTATION_SCHEDULED' }
      })
    }

    // Log da criação

    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_CREATED',        action: 'create_consultation',        clientId: validatedData.clientId,        details: {,          timestamp: new Date().toISOString(),          action: 'automated_action'
        },        success: true
      }
    }),
  ...
  if (error instanceof z.ZodError) {,      return NextResponse.json(,        { ,          error: 'Dados inválidos',          details: error.errors
    ...
    console.error('Erro ao criar consultoria:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}