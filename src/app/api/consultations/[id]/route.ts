...
// PATCH /api/consultations/[id] - Atualizar campo específico (inline edit),export async function PATCH(,  request: NextRequest,  { params }: { params: { id: string } }
    ...
    if (validatedData.scheduledAt) {
      ...
      if (validatedData.scheduledAt.includes('T')) {,        updateData.scheduledAt = new Date(validatedData.scheduledAt)
    ...
    const updatedConsultation = await prisma.consultation.update({,      where: { id },      data: updateData,      include: {,        client: {,          select: { id: true, name: true, email: true }
    ...
    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_UPDATED',        action: 'inline_edit',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString(),          action: 'automated_action'
        },        success: true
      }
    }),
...
export async function PUT(
  request: NextRequest,  { params }: { params: { id: string } }
    ...
    const existingConsultation = await prisma.consultation.findUnique({,      where: { id },      include: { client: true }
    }),
    if (!existingConsultation) {,      return NextResponse.json(,        { status: 404 }
      )
    }
    ...
    const updatedConsultation = await prisma.consultation.update({,      where: { id },      data: updateData,      include: {,        client: {,          select: { ,            id: true, ,            name: true, ,            email: true, ,            status: true 
          }
        },        consultant: {,          select: { id: true, name: true, email: true }
        }
      }
    ...
    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_UPDATED',        action: 'update_consultation',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString(),          action: 'automated_action'
        },        success: true
      }
    }),
    return NextResponse.json({,      data: updatedConsultation
...
export async function DELETE(
  request: NextRequest,  { params }: { params: { id: string } }
    ...
    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_DELETED',        action: 'delete_consultation',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString(),          action: 'automated_action'
        },        success: true
      }
    }),
