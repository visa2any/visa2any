...
// PATCH /api/consultations/[id] - Atualizar campo específico (inline edit),export async function PATCH(
  request: NextRequest,  { params }: { params: { id: string } }
) {,  try {
    // Verificar autenticação,    const user = await verifyAuth(request),    if (!user) {
      return createAuthError('Acesso não autorizado')
    ...
    // Se está marcando como completa, definir completedAt automaticamente,    if (validatedData.status === 'COMPLETED' && !updateData.completedAt) {
      updateData.completedAt = new Date()
    }

    // Atualizar consultoria,    const updatedConsultation = await prisma.consultation.update({,      where: { id },      data: updateData,      include: {,        client: {,          select: { id: true, name: true
 email: true }
    ...
    // Log da atualização,    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_UPDATED',        action: 'inline_edit',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString()
...
export async function PUT(
  request: NextRequest,  { params }: { params: { id: string } }
    ...
    // Log da atualização,    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_UPDATED',        action: 'update_consultation',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString()
          action: 'automated_action'
        },        success: true
      }
    }),
...
export async function DELETE(
  request: NextRequest,  { params }: { params: { id: string } }
    ...
    // Log da deleção,    await prisma.automationLog.create({,      data: {,        type: 'CONSULTATION_DELETED',        action: 'delete_consultation',        clientId: existingConsultation.clientId,        details: {,          timestamp: new Date().toISOString()
          action: 'automated_action'
        },        success: true
      }
    }),
