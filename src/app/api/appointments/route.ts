...
const visaType = searchParams.get('visaType')
    ...
    return NextResponse.json({,      slots,      total: slots.length,      consulate,      visaType,      message: slots.length > 0 
        ? `${slots.length} vagas encontradas` 
        : 'Nenhuma vaga disponível no período'
    })

  } catch (error) {,    console.error('Erro ao buscar vagas:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
try {
    ...
    if (!body[field as keyof BookingRequest]) {,        return NextResponse.json(,          { error: `Campo ${field} é obrigatório` },          { status: 400 }
        )
      }
    }
    ...
    } else {,      return NextResponse.json(,        { ,          error: bookingResult.error,          message: 'Não foi possível realizar o agendamento'
        },        { status: 400 }
      )
    }

  } catch (error) {,    console.error('Erro ao fazer agendamento:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
const { appointmentId, newDate, newTime, consulate } = body,
    ...
    const result = await appointmentBookingService.rescheduleAppointment(,      appointmentId,      newDate,      newTime,      consulate
    ),
    if (result.success) {,      return NextResponse.json({,        appointment: {,          id: result.appointmentId,          confirmationCode: result.confirmationCode,          date: result.date,          time: result.time,          location: result.location
  ...
  } catch (error) {,    console.error('Erro ao reagendar:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}