...
// POST /api/advisory/engine - Consultar advisory engine,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = advisoryQuerySchema.parse(body)
...
// GET /api/advisory/engine/countries - Listar países suportados,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const includeStats = searchParams.get('includeStats') === 'true',
...
// Realizar análise de elegibilidade,async function performEligibilityAssessment(profile: any, expertise: any) {,  const assessment = {,    overallScore: 0,    eligibilityLevel: 'low' as 'low' | 'medium' | 'high',    breakdown: {} as any,    blockers: [] as string[],    strengths: [] as string[]
  ...
  const country = profile.targetCountry,  const visaType = profile.visaType,  const expertiseData = expertise.data
