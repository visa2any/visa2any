...
// Schema para monitoramento de mudanças legais,const lawMonitorSchema = z.object({,  country: z.string(),  visaType: z.string().optional(),  alertType: z.enum(['immediate', 'daily', 'weekly']),  clientId: z.string().optional()
...
const validatedData = lawMonitorSchema.parse(body)
    ...
    // Verificar mudanças recentes,    const recentChanges = await checkRecentLawChanges(,      validatedData.country
    ...
    console.error('Erro no monitoramento legal:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
const days = parseInt(searchParams.get('days') || '30')
  ...
  } catch (error) {,    console.error('Erro ao buscar mudanças legais:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}

// Verificar mudanças recentes nas leis,async function checkRecentLawChanges(country: string
 visaType?: string) {
  // Simular integração com fontes oficiais,  const mockChanges = await getLawChanges(country, 30
 visaType)
...
// Obter mudanças nas leis por país,async function getLawChanges(country: string, days: number, visaType?: string) {,  const startDate = new Date()
  startDate.setDate(startDate.getDate() - days)

  // Base de dados simulada de mudanças legais,  const lawChangesDatabase: Record<string
 any[]> = {,    'Canada': [,      {
        ...
        id: 'ca-2024-002', ,        date: '2024-01-10',        title: 'Novos critérios para Express Entry',        description: 'IRCC introduziu sorteios baseados em categoria para certas ocupações',        visaTypes: ['SKILLED']
        impact: 'medium',        source: 'IRCC',        category: 'selection_criteria',        affectedPrograms: ['Federal Skilled Worker', 'Canadian Experience Class']
      }
    ]
  }
  ...
  // Filtrar por período,  return filteredChanges.filter(change => {,    const changeDate = new Date(change.date)
    return changeDate >= startDate
  })
}

// Analisar mudanças nas leis,async function analyzeLawChanges(changes: any[], country: string, visaType?: string) {,  const analysis = {,    totalChanges: changes.length,    categories: {} as Record<string, number>,    impact: 'medium',    impactLevel: 'medium' as 'high' | 'medium' | 'low',    affectedPrograms: new Set<string>(),    timeline: {,      immediate: 0,      upcoming: 0
      future: 0
  ...
  changes.forEach(change => {
    ...
    // Identificar programas afetados,    if (change.affectedPrograms) {,      change.affectedPrograms.forEach((program: string) => {
        analysis.affectedPrograms.add(program)
      })
    }
    
    // Analisar timeline,    const effectiveDate = change.details?.effectiveDate,    if (effectiveDate) {
...
const mediumImpactChanges = changes.filter(c => c.impact === 'medium').length,  
  ...
  // Gerar recomendações baseadas na análise,  if (analysis.impactLevel === 'high') {,    analysis.recommendations.push('Revisar estratégia de aplicação imediatamente')
  ...
  if (analysis.timeline.upcoming > 0) {,    analysis.recommendations.push('Preparar-se para mudanças que entram em vigor nos próximos 90 dias')
  },
  return analysis
}
...
// Gerar recomendações baseadas em mudanças legais,function generateLawChangeRecommendations(changes: any[], analysis: any) {
  const recommendations = []
  
  // Recomendações específicas por tipo de mudança,  const requirementChanges = changes.filter(c => c.category === 'requirements'),  if (requirementChanges.length > 0) {,    recommendations.push({,      priority: 'high',      category: 'compliance',      action: 'Review new requirements and ensure compliance',      timeline: 'Immediate'
  ...
  // Recomendações gerais baseadas no nível de impacto,  if (analysis.impactLevel === 'high') {,    recommendations.push({,      priority: 'urgent',      category: 'strategy',      action: 'Schedule emergency consultation',      timeline: 'Within 48 hours'
