...
// POST /api/advisory/law-monitor - Configurar monitoramento,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = lawMonitorSchema.parse(body)
...
// GET /api/advisory/law-monitor/changes - Obter mudanças recentes,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const country = searchParams.get('country'),    const days = parseInt(searchParams.get('days') || '30'),    const visaType = searchParams.get('visaType'),
...
// Analisar mudanças nas leis,async function analyzeLawChanges(changes: any[], country: string, visaType?: string) {,  const analysis = {,    totalChanges: changes.length,    categories: {} as Record<string, number>,    impact: 'medium',    impactLevel: 'medium' as 'high' | 'medium' | 'low',    affectedPrograms: new Set<string>(),    timeline: {,      immediate: 0,      upcoming: 0,      future: 0
  ...
  // Determinar nível de impacto geral,  const highImpactChanges = changes.filter(c => c.impact === 'high').length,  const mediumImpactChanges = changes.filter(c => c.impact === 'medium').length,  
...
// Configurar alertas de mudanças legais,async function setupLawChangeAlerts(data: any) {
