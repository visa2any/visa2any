...
// POST /api/advisory/compliance - Verificar compliance,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = complianceCheckSchema.parse(body)
...
// GET /api/advisory/compliance/requirements - Obter requisitos por país/visto,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const country = searchParams.get('country'),    const visaType = searchParams.get('visaType'),    const detailed = searchParams.get('detailed') === 'true',
...
// Realizar verificação de compliance,async function performComplianceCheck(documents: any[], requirements: any, checkType: string) {,  const result = {,    overallScore: 0,    complianceLevel: 'non_compliant' as 'compliant' | 'partial' | 'non_compliant',    issues: [] as any[],    passed: [] as any[]
    ...
    for (const req of requirements.mandatory) {,      const matchingDocs = documents.filter(doc => ,        doc.type === req.type || ,        doc.type.includes(req.type) ||,        req.type.includes(doc.type)
      ...
      if (matchingDocs.length === 0) {,        result.missing.push({,          type: req.type,          name: req.name,          severity: 'critical',          description: `Required document missing: ${req.name}`
        ...
        // Verificar cada documento correspondente,        for (const doc of matchingDocs) {,          const docCheck = await checkDocumentCompliance(doc, req),          
          ...
          // Verificar validade,          if (doc.expiryDate) {,            const expiryDate = new Date(doc.expiryDate),            const now = new Date(),            const sixMonthsFromNow = new Date(now.getTime() + 6 * 30 * 24 * 60 * 60 * 1000),            
  ...
  // Calcular score geral,  const mandatoryWeight = 0.8,  const optionalWeight = 0.2,  
  result.overallScore = (
...
// Verificar compliance de documento individual,async function checkDocumentCompliance(document: any, requirement: any) {,  const check = {,    compliant: true,    issues: [] as any[]
  ...
  // Verificar especificações técnicas,  if (requirement.specifications) {,    const specs = requirement.specifications
    ...
    // Verificar resolução (se disponível nos metadados),    if (specs.resolution && document.metadata?.resolution) {,      const requiredDPI = parseInt(specs.resolution),      const docDPI = parseInt(document.metadata.resolution),      
