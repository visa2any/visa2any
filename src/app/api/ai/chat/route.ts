...
// POST /api/ai/chat - Conversar com Sofia IA,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = chatMessageSchema.parse(body)
    ...
    return NextResponse.json({,      data: {,        message: sofiaResponse.message,        intent: sofiaResponse.intent,        confidence: sofiaResponse.confidence,        suggestions: sofiaResponse.suggestions,        actions: sofiaResponse.actions,        conversationId: sofiaResponse.conversationId
      }
    })
    ...
    console.error('Erro no chat com Sofia:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}

// GET /api/ai/chat/intents - Listar intenções disponíveis,export async function GET(request: NextRequest) {,  try {,    const intents = getSofiaIntents(),
    return NextResponse.json({,      data: { intents }
    })

  } catch (error) {,    console.error('Erro ao buscar intenções:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
// Detectar intenção da mensagem,function detectIntent(message: string) {,  const lowercaseMessage = message.toLowerCase(),  const intents = getSofiaIntents(),  
  let bestMatch: { name: string; confidence: number; keywords: string[] } = { name: 'unknown', confidence: 0, keywords: [] },  
  for (const [intentName, intentData] of Object.entries(intents)) {,    let score = 0,    const matchedKeywords: string[] = []
...
// Gerar resposta da Sofia,async function generateSofiaResponse(intent: any, message: string, clientContext: any, context: any) {,  const intentName = intent.name,  const clientName = clientContext?.name?.split(' ')[0] || 'Cliente',  
  switch (intentName) {,    case 'greeting':,      return {,        message: `Olá ${clientName}! 👋 Eu sou a Sofia, sua assistente virtual especializada em vistos e imigração. Como posso te ajudar hoje?`,        suggestions: [,          'Quero analisar minha elegibilidade',          'Quais documentos preciso?',          'Quanto custa o processo?',          'Quanto tempo demora?'
    ...
    case 'eligibility_question':,      if (clientContext) {,        const targetCountry = clientContext.targetCountry || context?.targetCountry,        if (targetCountry) {,          return {,            message: `Perfeito ${clientName}! Vejo que você está interessado em ${targetCountry}. ,
...
Gostaria de fazer uma análise mais detalhada? Posso te ajudar a:,1. Calcular sua elegibilidade completa,2. Mostrar os documentos necessários,3. Estimar timeline e custos`,            suggestions: [,              'Fazer análise completa',              'Ver documentos necessários',              'Conhecer os custos',              'Falar com especialista'
            ]
            actions: [{,              type: 'start_analysis',              label: 'Iniciar Análise Completa',              clientId: clientContext.id
            }]
          }
        }
      },      return {,        message: `Claro! Para analisar sua elegibilidade, preciso conhecer melhor seu perfil. ,
Vamos começar com algumas perguntas:,
1. **Para qual país você quer imigrar?** 🌍,2. **Qual sua idade?** 👤,3. **Qual seu nível de educação?** 🎓,4. **Quantos anos de experiência profissional você tem?** 💼,
Essas informações me ajudam a dar uma análise mais precisa!`,        suggestions: [,          'Canadá',          'Austrália', ,          'Portugal',          'Estados Unidos'
        ]
        actions: []
      },    
    case 'documents_question':,      const country = clientContext?.targetCountry || context?.targetCountry || extractCountryFromMessage(message),      if (country) {,        return await getDocumentsResponse(country, clientName)
      },      return {,        message: `Para te ajudar com os documentos, preciso saber para qual país você está aplicando. ,
...
Para qual país você está interessado?`,        suggestions: ['Canadá', 'Austrália', 'Portugal', 'Estados Unidos']
        actions: []
      },    
    case 'cost_question':,      return {,        message: `Ótima pergunta ${clientName}! Os custos variam dependendo do país e tipo de serviço. 
...
Qual pacote faz mais sentido para você?`,        suggestions: [,          'Consulta Básica - R$ 299',          'Consulta Premium - R$ 599', ,          'Serviço VIP - R$ 1.299',          'Preciso de mais detalhes'
        ]
        actions: [{,          type: 'show_pricing',          label: 'Ver Detalhes dos Pacotes'
        }]
      },    
    case 'timeline_question':,      return {,        message: `O tempo varia bastante por país ${clientName}! Aqui está um resumo:
...
Para qual país você está pensando?`,        suggestions: [,          'Portugal - mais rápido',          'Canadá - boa opção',          'Austrália - quero detalhes',          'Estados Unidos'
        ]
        actions: []
      },    
    case 'contact_human':,      return {,        message: `Claro ${clientName}! Vou te conectar com um de nossos especialistas. 
...
Qual opção prefere?`,        suggestions: [,          'Ligar agora',          'Agendar consultoria',          'WhatsApp',          'Email'
        ]
        actions: [{,          type: 'contact_specialist',          label: 'Falar com Especialista Agora'
        }]
      },    
    case 'complaint':,      return {,        message: `${clientName}, lamento muito pelo inconveniente! 😔,
...
💡 **Enquanto isso:**,Pode me dar mais detalhes sobre o problema? Assim posso já adiantar a solução.`,        suggestions: [,          'Problema com documento',          'Demora no atendimento', ,          'Cobrança indevida',          'Falar com gerente'
        ]
        actions: [{,          type: 'escalate_complaint',          label: 'Falar com Gerente Agora'
        }]
      },    
    default:,      return {,        message: `Entendi ${clientName}! Embora eu não tenha certeza total sobre isso, posso te ajudar com:
...
Posso reformular isso para uma dessas áreas? Ou prefere falar diretamente com um especialista humano?`,        suggestions: [,          'Analisar elegibilidade',          'Ver documentos necessários',          'Consultar custos',          'Falar com especialista'
        ]
        actions: []
      }
  }
...
// Obter resposta sobre documentos por país,async function getDocumentsResponse(country: string, clientName: string) {,  const countryLower = country.toLowerCase()
  ...
  if (requirements) {,    const docs = requirements.requiredDocuments as any[],    const docList = docs.map(doc => `- ${doc.name} (${doc.required ? 'Obrigatório' : 'Opcional'})`).join('\n'),    
    return {,      message: `Perfeito ${clientName}! Aqui estão os documentos para ${country}:
...
Quer uma análise completa do seu perfil?`,      suggestions: [,        'Fazer análise completa',        'Ver dicas de aprovação',        'Agendar consultoria',        'Outro país'
      ]
      actions: [{,        type: 'start_analysis',        label: 'Iniciar Análise Completa'
      }]
    }
  }
...
💡 **Recomendação:** Faça nossa análise IA gratuita para ter uma lista personalizada dos documentos exatos que você precisa!`,    suggestions: [,      'Análise IA gratuita',      'Falar com especialista',      'Ver outros países',      'Mais informações'
    ]
    actions: [{,      type: 'start_analysis',      label: 'Análise Gratuita Agora'
    }]
  }
}
...
// Intenções da Sofia IA,function getSofiaIntents() {,  return {,    greeting: {,      keywords: ['olá', 'oi', 'hello', 'hi', 'bom dia', 'boa tarde', 'boa noite'],      patterns: ['^(olá|oi|hello|hi)']
    ...
    documents_question: {,      keywords: ['documentos', 'papéis', 'preciso', 'necessário', 'documentação']
      patterns: ['que documentos', 'preciso.*documento', 'documentos.*necessário']
    },    
    cost_question: {,      keywords: ['custa', 'preço', 'valor', 'quanto', 'custo', 'investimento', 'taxa']
      patterns: ['quanto.*custa', 'qual.*preço', 'valor.*processo']
    },    
    timeline_question: {,      keywords: ['tempo', 'demora', 'duração', 'prazo', 'quanto tempo', 'timeline']
      patterns: ['quanto.*tempo', 'tempo.*demora', 'prazo.*processo']
    },    
    contact_human: {,      keywords: ['humano', 'pessoa', 'especialista', 'consultor', 'atendente', 'falar']
      patterns: ['falar.*humano', 'pessoa.*real', 'especialista.*humano']
    },    
    complaint: {,      keywords: ['problema', 'reclamação', 'erro', 'ruim', 'insatisfeito', 'demora', 'lento']
