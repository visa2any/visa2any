...
export async function POST(request: NextRequest) {
  try {
    ...
    if (client) {
      ...
      client = await prisma.client.update({
        ...
        data: {
          ...
          score: score,
          notes: JSON.stringify({ responses, category, priority }),
          status: score >= 75 ? 'QUALIFIED' : 'LEAD',
          updatedAt: new Date()}})} else {
      // Criar novo cliente
    ...
    if (category === 'hot') {
      ...
      await createFollowUpTask(client.id, 'SCHEDULE_CONSULTATION', 'Agendar consultoria premium', 2)} else if (category === 'warm') {
      ...
      await createFollowUpTask(client.id, 'OFFER_AI_ANALYSIS', 'Oferecer análise IA gratuita', 2)} else {
    ...
    if (category === 'hot') {
      await fetch('/api/automation/email-sequences', {
        ...
        body: JSON.stringify({
          ...
          ...automationData})})} else if (category === 'warm') {
      await fetch('/api/automation/email-sequences', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
