...
const { email, name, phone, responses, score, category, priority, nextAction } = body,
    ...
    if (client) {
      ...
      client = await prisma.client.update({,        where: { id: client.id },        data: {,          name: name,          phone: phone || client.phone,          eligibilityScore: score,          leadQualificationData: JSON.stringify(responses),          leadScore: score,          leadCategory: category.toUpperCase(),          leadPriority: priority.toUpperCase(),          status: score >= 75 ? 'QUALIFIED' : 'LEAD',          updatedAt: new Date()
        }
      })
    } else {
      // Criar novo cliente
      client = await prisma.client.create({,        data: {,          name,          email,          phone: phone || null,          eligibilityScore: score,          leadQualificationData: JSON.stringify(responses),          leadScore: score,          leadCategory: category.toUpperCase(),          leadPriority: priority.toUpperCase(),          status: score >= 75 ? 'QUALIFIED' : 'LEAD',          isActive: true,          destinationCountry: responses.country || null,          visaType: responses['visa-type'] || null,          createdAt: new Date(),          updatedAt: new Date()
        }
      })
    }
    ...
    await prisma.interaction.create({,      data: {,        clientId: client.id,        type: 'EMAIL',        channel: 'form',        direction: 'inbound',        subject: 'Lead Qualification',        content: `Lead qualification completed with score ${score}/100`,        completedAt: new Date()
      }
    })
    ...
    if (category === 'hot') {
      ...
      await createFollowUpTask(client.id, 'IMMEDIATE_CALL', 'Ligar imediatamente - Lead quente', 1),      await createFollowUpTask(client.id, 'SCHEDULE_CONSULTATION', 'Agendar consultoria premium', 2)
    } else if (category === 'warm') {
      // Lead morno - email personalizado + análise IA
      await createFollowUpTask(client.id, 'PERSONALIZED_EMAIL', 'Enviar email personalizado', 1),      await createFollowUpTask(client.id, 'OFFER_AI_ANALYSIS', 'Oferecer análise IA gratuita', 2)
    } else {
      // Lead frio - nurturing com conteúdo
      await createFollowUpTask(client.id, 'SEND_LEAD_MAGNETS', 'Enviar materiais educativos', 1),      await createFollowUpTask(client.id, 'ADD_TO_NURTURING', 'Adicionar à sequência de nurturing', 2)
    }
...
const automationData = {,      clientId: client.id,      email: client.email,      name: client.name,      category,      responses,      destinationCountry: responses.country,      urgency: responses.urgency,      budget: responses.budget
    ...
    if (category === 'hot') {,      await fetch('/api/automation/email-sequences', {,        method: 'POST',        headers: { 'Content-Type': 'application/json' },        body: JSON.stringify({,          sequence: 'hot_lead_immediate'
          ...automationData
        })
      })
