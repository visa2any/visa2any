import { NextRequest, NextResponse } from 'next/server',import { loginCustomer, loginAdmin } from '@/lib/auth-unified',
export async function POST(request: NextRequest) {,  try {,    const { email, password, type } = await request.json(),
  ...
  } catch (error) {,    console.error('Erro no login unificado:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}