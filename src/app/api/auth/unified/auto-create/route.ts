...
const { name, email, phone, country, nationality, targetCountry, source, product, amount } = data,
    ...
    const result = await createCustomerAccount({,      name,      email,      phone,      country,      nationality,      targetCountry,      source,      product,      amount
    }),
    ...
    const response = NextResponse.json({,      user: result.user,      token: result.token,      message: 'Conta criada e login automático realizado'
    })
  ...
  } catch (error) {,    console.error('Erro na criação automática de conta:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}