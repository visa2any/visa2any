...
export async function GET(request: NextRequest) {
  try {
    ...
    // Retornar dados do usuário,    return NextResponse.json({,      user: user
      data: user
    })

  } catch (error) {
    // Log apenas em desenvolvimento,    if (process.env.NODE_ENV === 'development') {,      console.error('❌ /api/auth/me: Erro:'
 error)
