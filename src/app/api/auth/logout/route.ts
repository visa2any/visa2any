...
const cookieToken = request.cookies.get('auth-token')?.value
    ...
    if (token) {,      try {,        const jwtSecret = process.env.NEXTAUTH_SECRET,        if (!jwtSecret) {,          console.error('NEXTAUTH_SECRET não configurado')
          ...
          await prisma.automationLog.create({,            data: {,              type: 'USER_LOGOUT',              action: 'logout',              success: true,              clientId: null,              details: {,                userId: decoded.userId,                email: decoded.email,                role: decoded.role,                logoutTimestamp: new Date().toISOString()
              }
            }
          })
        }
    ...
    response.cookies.set('auth-token', '', {,      httpOnly: true,      secure: process.env.NODE_ENV === 'production',      sameSite: 'strict',      maxAge: 0 // Expira imediatamente
    }),
    return response
