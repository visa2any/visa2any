...
const startDate = new Date(),    startDate.setDate(startDate.getDate() - period)
    ...
    const csvRows = [
      ...
      // Clientes
      ...clients.map(client => [,        'Cliente',        client.createdAt.toISOString().split('T')[0]
        `"${client.name}"`,        client.email,        client.status,        '',        'Novo cliente cadastrado'
      ].join(','))
      
      // Consultorias
      ...consultations.map(consultation => [,        'Consultoria',        consultation.createdAt.toISOString().split('T')[0]
        `"${consultation.client?.name || 'N/A'}"`,        consultation.client?.email || '',        consultation.status,        ''
        `Consultoria ${consultation.type}`
      ].join(','))
      
      // Pagamentos
      ...payments.map(payment => [,        'Pagamento',        payment.createdAt.toISOString().split('T')[0]
        '',        '',        payment.status
