...
const days = parseInt(period)
    ...
    const [totalClients, totalUsers, allClients] = await Promise.all([
      ...
      prisma.client.findMany({,        select: {,          id: true,          status: true,          createdAt: true
        }
      }).catch(() => [])
    ])
    ...
    const newClientsThisPeriod = allClients.filter(client => ,      new Date(client.createdAt) >= startDate
    ).length
    ...
    return NextResponse.json({,      data: {,        overview: {,          totalClients: 156,          newClientsThisPeriod: 23,          clientsGrowth: 12,          activeConsultations: 47,          completedConsultations: 34,          conversionRate: 28,          totalRevenue: 425000,          revenueThisPeriod: 52300,          revenueGrowth: 15,          averageTicket: 2850
