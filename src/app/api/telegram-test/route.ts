...
const text = update.message.text || 'Sem texto',          
    ...
    return NextResponse.json({,      success: true,      token: `${token.substring(0, 10)}...`,      totalUpdates: data.result?.length || 0,      uniqueChatIds: Array.from(chatIds),      messages: messages.slice(-5), // Últimas 5 mensagens
      rawData: data
    })
...
export async function POST(request: NextRequest) {,  try {,    const { chatId, message } = await request.json(),    const token = process.env.TELEGRAM_BOT_TOKEN,    
    ...
    // Enviar mensagem de teste,    const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {,      method: 'POST'
