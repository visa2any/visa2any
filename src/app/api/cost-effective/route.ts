import { NextRequest, NextResponse } from 'next/server',import { costEffectiveSolutions, ManualBookingRequest } from '@/lib/cost-effective-solutions'

// POST - Agendamento manual assistido (gratuito),export async function POST(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const action = searchParams.get('action') || 'manual_booking',    const body = await request.json(),
...
// GET - Listar métodos econômicos e calcular ROI,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const action = searchParams.get('action') || 'methods',
    switch (action) {,      case 'methods':
      ...
      case 'roi':
        // Calcular ROI de um método específico,        const method = searchParams.get('method'),        const monthlyVolume = parseInt(searchParams.get('volume') || '10'),        const revenuePerBooking = parseInt(searchParams.get('revenue') || '100'),        
