...
const result = await medicalExamService.getExamStatus(bookingId),
  ...
  } catch (error) {,    console.error('Erro ao verificar status do exame:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
const body = await request.json()
    ...
    if (result.success) {,      return NextResponse.json({,        confirmationCode: result.confirmationCode,        message: result.message
      })
    } else {,      return NextResponse.json(,      { error: 'Dados inválidos' },      { status: 400 }
    )
    }

  } catch (error) {,    console.error('Erro ao reagendar exame:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
const result = await medicalExamService.cancelExam(bookingId),
  ...
  } catch (error) {,    console.error('Erro ao cancelar exame:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}