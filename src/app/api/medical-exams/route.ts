...
const state = searchParams.get('state'),
    ...
    if (!country) {,        return NextResponse.json(,          { error: 'Parâmetro country é obrigatório para buscar clínicas' },          { status: 400 }
      ...
      return NextResponse.json({,        clinics,        total: clinics.length,        message: clinics.length > 0 
    ...
    if (!country) {,        return NextResponse.json(,          { error: 'Parâmetro country é obrigatório para buscar exames' },          { status: 400 }
      ...
      return NextResponse.json({,        exams,        total: exams.length,        message: exams.length > 0 
          ? `${exams.length} tipos de exame requeridos para ${country}` 
          : `Nenhum exame específico encontrado para ${country}`
      })
    },
    return NextResponse.json(,      { error: 'Parâmetro action deve ser "clinics" ou "exams"' },      { status: 400 }
    )

  } catch (error) {,    console.error('Erro ao buscar dados médicos:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
const { ,      applicantId ,      clinicId, ,      examTypes, ,      appointmentDate, ,      appointmentTime, ,      totalCost,      notes 
    ...
    if (!applicantId || !clinicId || !examTypes || !appointmentDate || !appointmentTime) {,      return NextResponse.json(,        { error: 'Campos applicantId, clinicId, examTypes, appointmentDate e appointmentTime são obrigatórios' },        { status: 400 }
      )
    },
    ...
    const result = await medicalExamService.bookMedicalExam({,      applicantId,      clinicId,      examTypes,      appointmentDate,      appointmentTime,      totalCost,      notes
    }),
  ...
  } catch (error) {,    console.error('Erro ao agendar exame médico:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}