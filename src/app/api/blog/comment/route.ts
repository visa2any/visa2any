import { NextRequest, NextResponse } from 'next/server',import { prisma } from '@/lib/prisma',import jwt from 'jsonwebtoken',import { z } from 'zod',
const commentSchema = z.object({,  postId: z.string().min(1, 'Post ID é obrigatório'),  content: z.string().min(1, 'Conteúdo é obrigatório').max(1000, 'Máximo 1000 caracteres'),  parentId: z.string().optional() // For replies
})

// POST /api/blog/comment - Add comment to blog post,export async function POST(request: NextRequest) {,  try {
    ...
    const body = await request.json(),    const validatedData = commentSchema.parse(body)
    ...
    console.error('Error adding blog comment:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}

// GET /api/blog/comment - Get comments for a blog post,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const postId = searchParams.get('postId'),
  ...
  } catch (error) {,    console.error('Error fetching blog comments:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}