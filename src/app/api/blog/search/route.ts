...
async function generateSuggestions(query: string) {,  const searchTerm = query.toLowerCase().trim(),  
  try {
    ...
    const countryMatches = await prisma.blogPost.findMany({,      where: {,        published: true,        country: {,          contains: searchTerm,          mode: 'insensitive'
        }
    ...
    const categoryMatches = await prisma.blogPost.findMany({,      where: {,        published: true,        category: {,          contains: searchTerm,          mode: 'insensitive'
        }
    ...
    titleMatches.forEach(post => {,      suggestions.push({,        type: 'post',        text: post.title,        category: post.category,        icon: '📄'
      })
    })
    ...
    categoryMatches.forEach(cat => {,      suggestions.push({,        type: 'category',        text: cat.category,        category: 'Categoria',        icon: '📂'
      })
    }),
...
const excerptLower = post.excerpt.toLowerCase()
      ...
      if (excerptLower.includes(queryLower)) {,        score += 50
      }

      // Score por país

      if (post.country && post.country.toLowerCase().includes(queryLower)) {,        score += 30
      }
    ...
    const uniqueResults = Array.from(,      new Map(resultsWithScore.map(post => [post.id, post])).values()
