...
export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const {,      title,      excerpt,      content,      category,      author,      tags,      country,      flag,      difficulty,      type,      sourceUrl,      urgent,      trending
    } = body
...
// Endpoint para verificar status do sistema,export async function GET() {,  try {,    const recentPosts = await prisma.blogPost.findMany({,      orderBy: { publishDate: 'desc' },      take: 5,      select: {,        id: true,        title: true,        publishDate: true,        author: true,        urgent: true,        trending: true
      }
    }),
...
// Função auxiliar para calcular tempo de leitura,function calculateReadTime(content: string): string {,  const wordsPerMinute = 200,  const words = content.split(' ').length,  const minutes = Math.ceil(words / wordsPerMinute),  return `${minutes} min`
}

// Endpoint para atualizar posts existentes,export async function PUT(request: NextRequest) {,  try {,    const body = await request.json(),    const { id, ...updateData } = body,
