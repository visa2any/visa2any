...
const { postId, action } = body,
    ...
    if (action === 'like') {
      ...
      if (!existingLike) {,        await prisma.blogPostLike.create({,          data: {,            userId,            postId
          }
        })
      }
    } else if (action === 'unlike') {,      await prisma.blogPostLike.deleteMany({,        where: {,          userId,          postId
        }
      })
    }
  ...
  } catch (error) {,    console.error('Error handling blog like:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}