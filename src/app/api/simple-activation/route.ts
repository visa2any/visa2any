...
Sistema pronto para produção!`)
      ...
      default:
        return NextResponse.json({
          error: 'Ação não reconhecida'}, { status: 400 })}
...
async function sendTelegramNotification(title: string, message: string) {
  ...
  try {
    ...
    if (response.ok) {
      console.log('Notificação Telegram enviada com sucesso')} else {
      console.error('Erro ao enviar notificação Telegram:', await response.text())}} catch (error) {
