...
export async function POST(request: NextRequest) {
  try {
    // Verificar authorization header se necessário,    const authHeader =  
const cronSecret = process.env.CRON_SECRET,    
  ...
  } catch (error) {,    console.error('❌ Erro no processamento de posts agendados:', error),    
    return NextResponse.json(,      { ,        error: 'Erro no processamento de posts agendados',        details: error instanceof Error ? error.message : 'Erro desconhecido'
      },      { status: 500 }
    )
  }
}
