...
const action = searchParams.get('action') || 'status'
    ...
    switch (action) {,      case 'status':
        ...
        return NextResponse.json({,          success: true,          automation: {,            playwright: {,              status: 'Ativo',              version: 'Latest',              browsers: ['Chromium', 'Firefox', 'Safari']
      ...
      case 'costs':
        ...
        return NextResponse.json({,          success: true,          pricing: {,            setup: {,              cost: 'R$ 0',              description: 'Playwright é gratuito e open source'
