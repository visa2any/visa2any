...
Att,Ana Silva,Consultora Sênior Visa2Any
📱 +55 11 99999-9999
      `
    },    {
      delay: 60
...
Ana Silva,Visa2Any
  ...
  warm_lead_nurturing: [,    {
      delay: 0,      subject: '✨ {name}, sua análise está pronta!',      template: `,Olá {name},
...
Ana Silva,Consultora Sênior
      `
    }
  ]

  cold_lead_education: [,    {
      delay: 0,      subject: '📚 {name}, comece sua jornada com conhecimento',      template: `,Olá {name}!,
...
const { sequence, clientId, email, name, category, responses, destinationCountry, urgency, budget } = body,
    ...
    const sendAt = new Date(),      sendAt.setMinutes(sendAt.getMinutes() + template.delay)
      ...
      await scheduleEmail({,        to: email,        subject: processedSubject,        body: processedBody,        sendAt,        sequence,        clientId,        templateIndex: templates.indexOf(template)
      })
    },
    return NextResponse.json({,      message: `Sequência ${sequence} ativada para ${email}`,      emailsScheduled: templates.length
    })

  } catch (error) {,    console.error('Erro ao processar sequência de emails:', error),    return NextResponse.json({,      error: 'Erro interno do servidor'
    }, { status: 500 })
  }
},
...
try {
    ...
    if (emailData.clientId) {,      await prisma.interaction.create({,        data: {,          clientId: emailData.clientId,          type: 'AUTOMATED_EMAIL',          channel: 'email',          direction: 'outbound',          subject: emailData.subject,          content: `Email agendado: ${emailData.subject}`,          completedAt: new Date()
        }
      })
    }

  } catch (error) {,    console.error('Erro ao agendar email:', error)
  }
},
...
try {
    ...
    const response = await fetch('/api/notifications/email', {,      method: 'POST',      headers: { 'Content-Type': 'application/json' },      body: JSON.stringify({,        to: emailData.to,        subject: emailData.subject,        html: emailData.body.replace(/\n/g, '<br>'),        template: 'automation',        variables: {,          content: emailData.body
        }
      })
    }),
    if (emailData.clientId) {,      await prisma.interaction.create({,        data: {,          clientId: emailData.clientId,          type: 'EMAIL',          channel: 'email',          direction: 'outbound',          subject: emailData.subject,          content: `Email enviado: ${emailData.subject}`,          completedAt: new Date()
        }
      })
    }
