...
const { email, password } = body,
    ...
    const customer = await prisma.client.findUnique({,      where: { email },      include: {,        consultations: {,          take: 1,          orderBy: { createdAt: 'desc' }
        }
      }
    }),
    ...
    const response = NextResponse.json({,      message: 'Login realizado com sucesso',      customer: {,        id: customer.id,        name: customer.name,        email: customer.email,        phone: customer.phone,        status: customer.status,        destinationCountry: customer.destinationCountry,        visaType: customer.visaType,        eligibilityScore: customer.eligibilityScore
      },      token
    })
