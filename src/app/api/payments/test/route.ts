...
export async function POST(request: NextRequest) {,  try {,    console.log('🧪 API de teste chamada'),    
    ...
    return NextResponse.json({,      preference_id: 'test-preference-' + Date.now(),      public_key: process.env.MERCADOPAGO_PUBLIC_KEY || 'TEST-PUBLIC-KEY',      message: 'Preferência de teste criada com sucesso'
    })
