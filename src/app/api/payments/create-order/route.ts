...
// POST /api/payments/create-order - Criar pedido,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = createOrderSchema.parse(body)
...
// Função para criar pagamento no Mercado Pago,async function createMercadoPagoPayment(orderData: {,  orderId: string,  title: string,  quantity: number,  unitPrice: number,  description: string,  clientEmail?: string
    ...
    // Configurar dados da preferência,    const preferenceData = {,      items: [,        {
      ...
      },      auto_return: 'approved',      notification_url: `${process.env.NEXTAUTH_URL}/api/payments/webhook/mercadopago`,      external_reference: orderData.orderId,      expires: true,      expiration_date_from: new Date().toISOString(),      expiration_date_to: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(), // 24 horas,      payment_methods: {,        excluded_payment_methods: []
    ...
    const preference = new Preference(client),    const result = await preference.create({ body: preferenceData }),    
