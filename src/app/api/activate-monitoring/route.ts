...
async function activateEmailMonitoring() {,  try {,    if (emailMonitoringActive) {,      return NextResponse.json({ ,        message: 'Email monitoring já está ativo',
    ...
    // Ativar email monitoring com intervalo de 15 minutos,    emailInterval = setInterval(async () => {,      try {,        console.log('📧 Verificando emails de consulados...'),        const [recentAlerts, consulateAlerts] = await Promise.all([
...
async function deactivateAll() {
  // Parar intervalos,  if (webScrapingInterval) {,    clearInterval(webScrapingInterval)
