...
export async function POST(request: NextRequest) {,  try {,    const { clientId, score, level, answers, recommendations, nextSteps } = await request.json(),
    ...
    await prisma.client.update({,      where: { id: clientId },      data: {,        score,        status: score >= 80 ? 'QUALIFIED' : 'LEAD'
      }
    })

    // Criar interação de análise realizada

    await prisma.interaction.create({,      data: {,        clientId,        type: 'AUTOMATED_EMAIL',        channel: 'system',        direction: 'inbound',        subject: 'Análise de Elegibilidade Realizada',        content: `Score calculado: ${score}% - Nível: ${level}`,        completedAt: new Date()
      }
    })
    ...
    await prisma.automationLog.create({,      data: {,        clientId,        type: 'ANALYSIS_COMPLETED',        action: `analysis_saved`,        success: true,        details: {,          score: score,          automationAction: automationAction,          analysisType: 'eligibility_analysis',          triggeredAt: new Date().toISOString()
        }
      }
    }),
