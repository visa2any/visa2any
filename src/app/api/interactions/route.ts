...
const type = searchParams.get('type')
    ...
    const [interactions, total] = await Promise.all([,      prisma.interaction.findMany({,        where,        skip,        take: limit,        orderBy: { createdAt: 'desc' },        include: {,          client: {,            select: { ,              id: true, ,              name: true, ,              email: true, ,              phone: true,              status: true
            }
          }
        }
    ...
    return NextResponse.json({,      data: {,        interactions,        pagination: {,          page,          limit,          total,          totalPages,          hasMore: page < totalPages
        }
      }
    })

  } catch (error) {,    console.error('Erro ao buscar interações:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}
...
try {
    ...
    await prisma.automationLog.create({,      data: {,        type: 'EMAIL',        action: 'create_interaction',        clientId: validatedData.clientId,        details: {,          timestamp: new Date().toISOString(),          action: 'automated_action'
        },        success: true
      }
    }),
  ...
  if (error instanceof z.ZodError) {,      return NextResponse.json(,        { ,          error: 'Dados inválidos',          details: error.errors
    ...
    console.error('Erro ao criar interação:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}