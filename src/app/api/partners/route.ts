import { NextRequest, NextResponse } from 'next/server',import { partnerIntegrationService, PartnerBookingRequest } from '@/lib/partner-integrations'

// GET - Listar parceiros disponíveis,export async function GET(request: NextRequest) {,  try {,    const { searchParams } = new URL(request.url),    const country = searchParams.get('country'),    const action = searchParams.get('action'),
    if (action === 'status') {
    ...
    // Buscar melhor parceiro para requisição,    const visaType = searchParams.get('visaType') || 'tourist',    const urgency = searchParams.get('urgency') || 'normal',    
...
// POST - Fazer agendamento via parceiro,export async function POST(request: NextRequest) {,  try {,    const body: PartnerBookingRequest = await request.json()

    // Validação dos campos obrigatórios,    const requiredFields = ['applicantInfo', 'visaInfo'],    for (const field of requiredFields) {,      if (!body[field]) {,        return NextResponse.json(,          { error: `Campo ${field} é obrigatório` },          { status: 400 }
        )
      }
    }

    // Validar informações do aplicante,    const requiredApplicantFields = ['fullName', 'email', 'nationality'],    for (const field of requiredApplicantFields) {,      if (!body.applicantInfo[field as keyof typeof body.applicantInfo]) {,        return NextResponse.json(,          { error: `Campo applicantInfo.${field} é obrigatório` },          { status: 400 }
        )
      }
    }

    // Validar informações do visto,    if (!body.visaInfo.country || !body.visaInfo.visaType) {,      return NextResponse.json(,        { error: 'Campos visaInfo.country e visaInfo.visaType são obrigatórios' },        { status: 400 }
      )
    }

    // Se partnerId não foi especificado, encontrar o melhor,    if (!body.partnerId) {,      const bestPartner = await partnerIntegrationService.findBestPartner(,        body.visaInfo.country,        body.visaInfo.visaType,        body.visaInfo.urgency
      ),
    ...
    if (result.success) {
      // Calcular custo total com margem,      const totalCost = partnerIntegrationService.calculateTotalCost(,        result.cost || 0,        body.visaInfo.urgency
      ),
