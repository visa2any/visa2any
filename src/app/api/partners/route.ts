...
export async function POST(request: NextRequest) {,  try {
    ...
    // Validação dos campos obrigatórios,    const requiredFields = ['applicantInfo', 'visaInfo'],    for (const field of requiredFields) {,      if (!body[field]) {
        return NextResponse.json(,          { error: `Campo ${field} é obrigatório` },          { status: 400 }
        )
      }
    }

    // Validar informações do aplicante,    const requiredApplicantFields = ['fullName', 'email', 'nationality'],    for (const field of requiredApplicantFields) {,      if (!body.applicantInfo[field as keyof typeof body.applicantInfo]) {
    ...
    // Validar informações do visto,    if (!body.visaInfo.country || !body.visaInfo.visaType) {
      return NextResponse.json(,        { error: 'Campos visaInfo.country e visaInfo.visaType são obrigatórios' },        { status: 400 }
      )
    }

    // Se partnerId não foi especificado, encontrar o melhor,    if (!body.partnerId) {,      const bestPartner = await partnerIntegrationService.findBestPartner(,        body.visaInfo.country,        body.visaInfo.visaType
    ...
    if (result.success) {
      // Calcular custo total com margem,      const totalCost = partnerIntegrationService.calculateTotalCost(,        result.cost || 0
        body.visaInfo.urgency
      ),
