import { NextRequest, NextResponse } from 'next/server',import { prisma } from '@/lib/prisma',import { getWhatsAppService } from '@/lib/whatsapp',import { z } from 'zod'
...
// POST /api/notifications/whatsapp - Enviar WhatsApp,export async function POST(request: NextRequest) {,  try {,    const body = await request.json(),    const validatedData = sendWhatsAppSchema.parse(body)
...
// GET /api/notifications/whatsapp/status - Obter status do WhatsApp,export async function GET(request: NextRequest) {,  try {,    const whatsappService = getWhatsAppService(),    const status = whatsappService.getStatus(),
