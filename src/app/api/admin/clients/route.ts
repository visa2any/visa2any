...
export async function POST(request: NextRequest) {,  try {,    const body = await request.json()
    ...
    const client = await prisma.client.create({,      data: {,        name,        email,        phone: phone || null,        profession: profession || null,        nationality: nationality || null,        targetCountry: targetCountry || null,        visaType: visaType || null,        status: 'LEAD',        score: 0,        notes: body.notes || null
      }
    }),
    return NextResponse.json({,      data: client
    })

  } catch (error) {,    console.error('❌ Erro ao criar cliente:', error),    return NextResponse.json(,      {
