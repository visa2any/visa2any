...
export async function POST(,  request: NextRequest,  { params }: { params: { id: string } }
) {,  try {,    const clientId = params.id,    const body = await request.json(),    const { content } = body,
    if (!clientId) {,      return NextResponse.json(,        { error: 'Dados inválidos' },        { status: 400 }
    )
    },
    if (!content || content.trim().length === 0) {,      return NextResponse.json(,        { error: 'Dados inválidos' },        { status: 400 }
    )
    }

    // Create note record,    const noteId = `note_${Date.now()}`,    const newNote = {,      id: noteId,      clientId,      content: content.trim(),      author: 'Current User', // In a real app, get from session,      createdAt: new Date().toISOString(),      updatedAt: new Date().toISOString()
    }

    // Here you would save the note to the database
    // For now, we'll simulate the save operation,    await new Promise(resolve => setTimeout(resolve, 200))
  ...
  } catch (error) {,    console.error('Add note error:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
},
export async function GET(,  request: NextRequest,  { params }: { params: { id: string } }
  ...
  } catch (error) {,    console.error('Get notes error:', error),    return NextResponse.json(,      { error: 'Erro interno do servidor' },      { status: 500 }
    )
  }
}