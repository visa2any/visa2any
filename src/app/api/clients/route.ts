...
const search = searchParams.get('search')
    ...
    // Buscar clientes com paginação,    const [clients, total] = await Promise.all([,      prisma.client.findMany({,        where,        skip,        take: limit,        orderBy: { createdAt: 'desc' },        include: {,          assignedUser: {,            select: { id: true, name: true
 email: true }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log da criação,    await prisma.automationLog.create({,      data: {,        type: 'CLIENT_CREATED',        action: 'create_client',        clientId: client.id,        details: {,          timestamp: new Date().toISOString()
