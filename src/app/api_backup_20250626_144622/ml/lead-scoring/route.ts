...
const leadScoringSchema = z.object({
  ...
  profile: z.object({
    // Dados demográficos,    age: z.number().optional()
    education: z.string().optional()
    income: z.string().optional()
    maritalStatus: z.string().optional()
    
    // Dados de interesse,    targetCountry: z.string().optional()
    visaType: z.string().optional()
    timeframe: z.string().optional()
    
    // Dados comportamentais,    source: z.string().optional()
    utmSource: z.string().optional()
    utmMedium: z.string().optional()
    deviceType: z.string().optional()
    
    // Engagement,    pageViews: z.number().optional()
    timeOnSite: z.number().optional()
    emailOpens: z.number().optional()
    clickThroughs: z.number().optional()
    
    // Interações,    assessmentCompleted: z.boolean().optional()
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Buscar dados históricos do cliente se disponível,    let clientData = null
    ...
    // Salvar score atualizado,    if (validatedData.clientId) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Buscar dados de scoring,    const scoringData = await prisma.automationLog.findMany({
      where: {
        type: 'ML_LEAD_SCORING'
...
function calculateDemographicScore(profile: any): number {
  ...
  // Idade (sweet spot: 25-45 anos),  if (profile.age) {
    if (profile.age >= 25 && profile.age <= 45) score += 5
    else if (profile.age >= 20 && profile.age <= 50) score += 3
    else score += 1
  }

  // Educação,  const educationScores: Record<string, number> = {
    'DOCTORATE': 5,
    'MASTER': 4,
    'BACHELOR': 3,
    'TECHNICAL': 2,
  ...
  // Renda,  const incomeScores: Record<string, number> = {
    '20000+': 5,
    '10000-20000': 4,
    '6000-10000': 3,
    '3000-6000': 2,
  ...
  // Estado civil (casados tendem a ser mais estáveis),  if (profile.maritalStatus === 'MARRIED') score += 3
...
function calculateBehavioralScore(profile: any, clientData: any): number {
  ...
  // Fonte de tráfego (qualidade),  const sourceScores: Record<string, number> = {
    'organic': 8,
    'referral': 10,
    'direct': 7,
    'social': 5,
  ...
  // UTM tracking (campanhas específicas são melhores),  if (profile.utmMedium === 'cpc') score += 3
  ...
  // Histórico de interações,  if (clientData?.interactions) {
...
function calculateEngagementScore(profile: any, clientData: any): number {
  ...
  // Page views (mais páginas = mais interesse),  if (profile.pageViews) {
    if (profile.pageViews > 10) score += 8
    else if (profile.pageViews > 5) score += 5
    else if (profile.pageViews > 2) score += 3
  }

  // Tempo no site (engagement profundo),  if (profile.timeOnSite) {
  ...
  // Visitas à página de preços (alta intenção),  if (profile.pricingPageVisits) {
...
function calculateIntentScore(profile: any, clientData: any): number {
  ...
  // Timeframe urgente,  const timeframeScores: Record<string, number> = {
    '6 meses': 5,
    '1 ano': 4,
    '2 anos': 2,
    '3+ anos': 1
...
function calculateFitScore(profile: any): number {
  ...
  // Países com alta taxa de aprovação,  const countryScores: Record<string, number> = {
    'Portugal': 5,
    'Canada': 4,
    'Australia': 4,
    'Germany': 3,
  ...
  // Tipos de visto mais viáveis,  const visaScores: Record<string, number> = {
    'SKILLED': 4,
    'INVESTMENT': 5,
    'WORK': 3,
    'STUDENT': 2,
...
function calculateConversionProbability(scoringResult: any, clientData: any): number {
  ...
  // Base no score,  const score = scoringResult.totalScore
  ...
  // Ajustes baseados em dados históricos,  if (clientData?.interactions?.length > 5) probability += 10
...
function calculateEstimatedLTV(profile: any, scoringResult: any): number {
  ...
  // LTV base por país,  const countryLTV: Record<string, number> = {
    'United States': 3500,
    'Australia': 2800,
    'Canada': 2500,
    'Portugal': 1200,
  ...
  // Multiplicadores baseados no perfil,  if (profile.income === '20000+') baseLTV *= 1.5
  ...
  // Ajuste pelo score,  const scoreMultiplier = 0.5 + (scoringResult.totalScore / 100)
...
async function calculateModelAccuracy(data: any[]) {
  // Calcular accuracy comparando previsões com conversões reais,  // Em produção
 implementar análise mais sofisticada
  return 78.5 // Placeholder
}

function analyzeConversionFactors(data: any[]) {
  // Analisar quais fatores mais correlacionam com conversão,  return [
...
function generateTrends(data: any[]) {
  // Gerar trends de score ao longo do tempo,  return {
